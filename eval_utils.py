...
def process_input_data(input_data, args,top_n,tokenizer):
    ...
    for item in input_data:
        # if "passage" not in item and "gold_passages" in item:
        #     item["passage"] = item["gold_passages"]
        passage_list = []
        passage_id_list = []
        if args.rerank:
        ...
        # if args.replug:
        #     replug_list=[]
        #     for psg in passage_list[:top_n]:
        #         replug_list.append('Background:\n' + psg + '\n\n' + item["instruction"])
        #     item["instruction"]=replug_list

        passage_list = passage_list[:top_n]
        passage_id_list = passage_id_list[:top_n]
        item["passage_id_list"] = passage_id_list
        replug_list = []
        for psg in passage_list:
            prompt = 'Background:\n' + psg + '\n\n' + item["instruction"]
            replug_list.append(prompt)
        item["instruction"]=replug_list
        # passage = '\n'.join(passage_list)
